...
mod tests {
    ...
    async fn test_add_user() {
        let user = User {
            email: "test@mytest.com".to_string(),
    ...
    async fn test_validate_user() {
        ...
        let add_result = store.add_user(user.clone()).await;
        assert!(add_result.is_ok());
        let validate_result = store.validate_user(&user.email, &user.password).await;
        assert!(validate_result.is_ok());
        let invalid_validate_result = store.validate_user(&user.email, "wrongpassword").await;
